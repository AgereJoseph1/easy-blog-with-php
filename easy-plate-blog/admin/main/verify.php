<?php
    ...
    if(isset($_GET['auth']) && !empty($_GET['auth']) && isset($_GET['token']) && !empty($_GET['token'])){
        ...
        if($user->rowCount() > 0){
            ...
            $user->query("UPDATE users SET is_verified = 'true' WHERE id = :id");
            ...
            if($user->execute()){
                ...
                $user->bind("token", $token);
                if($user->execute()){
                    echo "<center><h2>Email verified successfully</h2><br><a href='../../frontend.front-end/login.php'>Login Now</a></center>";
                }
            }
        }else{
