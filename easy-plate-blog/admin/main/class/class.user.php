...
class USER
{
  ...
  // public function file_process($name, $type, $path){
  //   $file_ = $_FILES[$name];
  //   if(!empty($file_['tmp_name'])){
  //       if($upload = file_upload($file_, $type, $path, $size=200000)){
  //           if($upload['status'] == "success"){
  //               ${$name."_filename"} = $upload['file_name'];
  ...
  //       }
  //   }else{
  //      return "Please choose a ".$name." photograph!";
  //   }
  // }
  ...
  // New class add 11/April/2021 by samim laskar

  // GET USER ROLE

  public function user_role($id){
    $this->query("SELECT role FROM users WHERE id = :id LIMIT 1");
    $this->bind("id", $id);
    return $this->fetchOne()['role'];
  }
 //Usages: $user->user_role($user->sessionID());
